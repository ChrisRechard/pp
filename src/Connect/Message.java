	...
	public Object JsonDecode(JSONObject json) {
		...
		try {
			...
			if (type.equals(Danmu.Code)) {
				...
				// 部分弹幕服务器没有这个。。。所以屏蔽掉
				// bamboo.mQid =
				// json.getJSONObject("data").getJSONObject("to").getInt("toqid");
				...
				gift.mContentPicChatUrl = json.getJSONObject("data").getJSONObject("content").getJSONObject("pic")
						.getJSONObject("pc").getString("chat");
				gift.mContentPicEffectUrl = json.getJSONObject("data").getJSONObject("content").getJSONObject("pic")
						.getJSONObject("pc").getString("effect");
